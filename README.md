...
从 HTML 格式的聊天记录导出文件中提取说话风格，生成人设 YAML 文件。
...
2. **企业微信安全对接**：完整实现 AES-CBC 消息加解密、SHA1 签名验证、access_token 自动缓存与刷新
3. **异步回复 + 去重**：线程池处理消息回复，消息锁防止重复响应
4. **多用户隔离**：每个用户独立的对话历史管理
